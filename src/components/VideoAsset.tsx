...
const VideoAsset: React.FC = () => {
  ...
  const onLoad = () => {
    ...
    mediaPlayerRef?.current?.addEventListener('loadedmetadata', onLoadedmetadata);
  ...
  const onDestroy = () => {
    ...
    mediaPlayerRef?.current?.removeEventListener('loadedmetadata', onLoadedmetadata);
  ...
  useEffect(() => {
    ...
    if (mediaPlayerRef.current) {
      ...
      if (isSafariBrowser) {
        ...
        const hlsSourceEl = mediaPlayerRef.current.querySelectorAll('source')[0] as HTMLSourceElement;
        ...
        hlsInstance.on(Hls.Events.FRAG_BUFFERED, (event, data) => {
          ...
          const downloadTimeMs = data.stats.loading.end - data.stats.loading.start;
  ...
  return (
    <video id={'video360'} ref={mediaPlayerRef} crossOrigin="anonymous" playsInline autoPlay>
      <source
        src={
